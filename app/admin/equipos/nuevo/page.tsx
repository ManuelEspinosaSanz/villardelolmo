...
import { ArrowLeft, Save, CheckCircle, AlertCircle, Loader2, Shield } from "lucide-react"
...
export default function NuevoEquipoPage() {
  ...
  useEffect(() => {
    async function loadCategorias() {
      ...
      const { data } = await supabase.from("categorias_equipo").select("*").order("orden")
      setCategorias(data || [])
      if (data && data.length > 0) {
        setFormData((prev) => ({ ...prev, categoria_id: prev.categoria_id || data[0].id }))
      }
    }
    loadCategorias()
  }, [])
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-heading font-bold text-zinc-900">Nuevo Equipo</h1>
          <p className="text-zinc-600">Crea un equipo del club</p>
        </div>
      </motion.div>
      ...
      {saved && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <span className="text-green-800 font-medium">Equipo creado correctamente. Redirigiendo...</span>
        </motion.div>
      )}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-xl border border-zinc-200 p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="grupo_liga"
                value={formData.grupo_liga}
                onChange={(e) => setFormData({ ...formData, grupo_liga: e.target.value })}
                placeholder="Ej: Grupo 7"
              />
            </div>
            <div className="space-y-2">
